...
describe("gateway e2e", () => {
  ...
  it(
    "writes the agent base prompt templates on startup",
    { timeout: GATEWAY_E2E_TIMEOUT_MS },
    async () => {
      const { envSnapshot, tempHome, workspaceDir } = await setupGatewayTempHome({
        prefix: "openclaw-agent-base-template-home-",
        minimalGateway: true,
      });

      const token = nextGatewayId("agent-template-token");
      ...
      const legacyAgentPromptPath = path.join(
        process.env.OPENCLAW_STATE_DIR!,
        "agents",
        "main",
        "agent",
      ...
      try {
        const agentTemplate = await fs.readFile(
          path.join(process.env.OPENCLAW_STATE_DIR!, "agent-base.md"),
          "utf8",
        );
        const legacyTemplate = await fs.readFile(
          path.join(process.env.OPENCLAW_STATE_DIR!, "app-server-base.md"),
          "utf8",
        );
        expect(agentTemplate).toContain("Generated by OpenClaw. Do not edit this file in place.");
        expect(agentTemplate).toContain("agent-base.md");
        expect(agentTemplate).toContain("You are a personal assistant running inside OpenClaw.");
        expect(agentTemplate).toContain("<persona_latch>");
        expect(legacyTemplate).toContain("legacy Codex app-server template alias");
        expect(legacyTemplate).toContain("agent-base.md");
        await expect(fs.stat(agentPromptPath)).rejects.toMatchObject({ code: "ENOENT" });
        await expect(fs.stat(legacyAgentPromptPath)).rejects.toMatchObject({ code: "ENOENT" });
      } finally {
        await server.close({ reason: "agent base template test complete" });
        await removeGatewayTempHome(tempHome);
        envSnapshot.restore();
      }
    },
