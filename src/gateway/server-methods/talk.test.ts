...
describe("talk.speak handler", () => {
  ...
  it("uses agent voice from talk.providers when agentId is provided", async () => {
    // This test verifies the fix for: per-agent TTS override was not being applied to talk.speak
    // because resolveConfigWithAgentTts only updated messages.tts, not talk.providers
    const runtimeConfig: OpenClawConfig = {
      talk: {
        provider: "openai",
        providers: {
          openai: {
            voiceId: "alloy", // Global voice
            apiKey: "openai-key",
          },
        },
      },
        ...
        list: [
          {
            ...
            tts: {
              providers: {
                openai: {
                  voice: "echo", // Agent-specific male voice
                },
              },
            },
          },
    ...
    mocks.loadConfig.mockReturnValue(runtimeConfig);
    // Mock a speech provider that exposes what config it received
    ...
    mocks.synthesizeSpeech.mockImplementation(async () => {
      // The voice should come from talk.providers (agent-scoped)
      return {
        success: true,
        provider: "openai",
        audioBuffer: Buffer.from([1, 2, 3]),
    ...
    expect(mocks.synthesizeSpeech).toHaveBeenCalledTimes(1);
    // The key assertion: talk provider config should have agent's voiceId
