...
describe("ACP working directory validation", () => {
  ...
  it("accepts remote ACP backends with non-host-local absolute cwd values", () => {
    const result = validateConfigObject({
      agents: {
        list: [
          {
  ...
  it("still rejects relative ACP cwd values for remote backends", () => {
    const result = validateConfigObject({
      agents: {
        list: [
          {
    ...
    if (!result.ok) {
      expect(result.issues).toEqual(
        expect.arrayContaining([
          expect.objectContaining({
            path: "agents.list.0.runtime.acp.cwd",
            message: expect.stringContaining("must be an absolute local path"),
          }),
        ]),
      );
    }
  ...
  it("loads remote ACP cwd values without host-local existence checks", async () => {
    await withTempHomeConfig(
      {
        agents: {
          list: [
