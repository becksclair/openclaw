...
function validateConfiguredAcpWorkingDirectories(config: OpenClawConfig): ConfigValidationIssue[] {
  ...
  const shouldValidateOnCurrentHost = (backend: string | undefined): boolean => {
    const normalized = normalizeLowercaseStringOrEmpty(backend);
    return normalized === "" || normalized === "acpx";
  };
  ...
  for (const [index, entry] of (config.agents?.list ?? []).entries()) {
    ...
    if (
      !validation.ok &&
      (shouldValidateOnCurrentHost(entry.runtime.acp?.backend) ||
  ...
  for (const [index, binding] of (config.bindings ?? []).entries()) {
    ...
    if (
      !validation.ok &&
      (shouldValidateOnCurrentHost(binding.acp?.backend) || validation.reason === "not_absolute")
    ) {
      issues.push({
