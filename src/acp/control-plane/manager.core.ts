...
export class AcpSessionManager {
  ...
  async initializeSession(input: AcpInitializeSessionInput): Promise<{
    ...
    return await this.withSessionActor(sessionKey, async () => {
      ...
      const initialRuntimeOptions = runtimeManagesCwd
        ? {}
        : validateRuntimeOptionPatch({ cwd: input.cwd });
  ...
  private async ensureRuntimeHandle(params: {
    ...
    const runtime = backend.runtime;
    const runtimeCapabilities = await this.resolveRuntimeCapabilities({ runtime });
    const requestedCwd = hasManagedRuntimeOption(runtimeCapabilities, "cwd") ? undefined : cwd;
    ...
    if (cached) {
      ...
      const cwdMatches = hasManagedRuntimeOption(runtimeCapabilities, "cwd")
        ? true
        : (cached.cwd ?? "") === (cwd ?? "");
    ...
    const now = Date.now();
    const effectiveCwd = normalizeText(ensured.cwd) ?? requestedCwd;
