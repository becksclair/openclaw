...
function resolveRuntimeConfigOptionAliases(key: string): readonly string[] {
  const normalizedKey = normalizeLowercaseStringOrEmpty(key);
  for (const [optionKey, aliases] of Object.entries(RUNTIME_CONFIG_OPTION_ALIASES)) {
    if (
      normalizeLowercaseStringOrEmpty(optionKey) === normalizedKey ||
      aliases.some((alias) => normalizeLowercaseStringOrEmpty(alias) === normalizedKey)
    ) {
      return aliases;
    }
  }
