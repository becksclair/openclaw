...
export class GatewayBrowserClient {
  ...
  private async buildConnectPlan(): Promise<ConnectPlan> {
    const role = this.opts.role ?? CONTROL_UI_OPERATOR_ROLE;
    const scopes =
      this.opts.scopes ??
      (role === CONTROL_UI_OPERATOR_ROLE ? [...CONTROL_UI_OPERATOR_SCOPES] : []);
    const caps = this.opts.caps ?? (role === CONTROL_UI_OPERATOR_ROLE ? ["tool-events"] : []);
