...
describe("CodexAppServerClient", () => {
  ...
  it("initializes with the required client version", async () => {
    ...
    harness.send({
      id: outbound.id,
      result: { userAgent: "openclaw/0.144.1 (macOS; test)" },
    });
  ...
  it("blocks same-version Codex app-server prereleases below the stable floor", async () => {
    ...
    harness.send({
      id: outbound.id,
      result: { userAgent: "openclaw/0.144.1-alpha.2 (macOS; test)" },
    });

    await expect(initializing).rejects.toThrow(
      `Codex app-server ${MIN_CODEX_APP_SERVER_VERSION} or newer is required, but detected 0.144.1-alpha.2`,
    );
    expect(harness.writes).toHaveLength(1);
  });
