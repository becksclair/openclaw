...
<AccordionGroup>
  ...
  <Accordion title="Talk and TTS">
    ...
    - `tts.speak` (`operator.write`) renders non-empty `text` with the configured general TTS provider chain and returns one whole clip inline as `audioBase64`, plus `provider` and optional `outputFormat`, `mimeType`, and `fileExtension` metadata. Unlike `tts.convert`, it does not return a Gateway-local path; unlike `talk.speak`, it does not require a Talk provider. Before synthesis, text above the effective `messages.tts.maxTextLength` and provider/model limit is summarized when summaries are enabled; disabled or failed summarization falls back to bounded truncation. Synthesis failures return `UNAVAILABLE`.
