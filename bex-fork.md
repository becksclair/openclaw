...
Reconciled 2026-07-22 after the private npm deployment repair: **Private root package dependency bundling** is a new support/proof carry that keeps registry-private deployment artifacts self-contained and rejects missing declared bundle payloads before publish. The current table now contains 47 active carry rows plus 3 absorbed-upstream records.
...
| Managed native sky-cua Codex plugins                                 | Runtime carry         | Critical   | Stable has no fail-closed consumer for the producer-owned `openai-bundled` marketplace. Carry pre-thread active-release resolution, exact-path native install/read verification for `computer-use` and `browser-use`, byte-and-mode-compatible installed-tree hashing, MCP collision/readiness gates, and release-keyed per-home single-flight.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
| Private root package dependency bundling                             | Support/proof carry   | High       | Private Gitea deploys install the root tarball against one registry, so patched `@openclaw/fs-safe` must travel inside the root artifact. Keep the root bundle declaration, generic declared-bundle tarball validation, and pre-publish Gitea check aligned.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
Carry behavior: every isolated native Codex agent home reconciles the producer-owned sky-cua compatibility marketplace before any `thread/start`. OpenClaw invokes the installed `sky-cua-release resolve-active` command, validates the exact schema-v1 `openai-bundled` contract, treats `codex_marketplace.path` as the verified containment root, and passes only the absolute `codex_marketplace.manifest_path` marketplace JSON file to pinned Codex `plugin/install` and `plugin/read`. Both `computer-use@openai-bundled` (`0.1.0-sky-cua`, MCP `computer-use`) and `browser-use@openai-bundled` (`1.0.0-sky-cua-openclaw`, MCP `node_repl`) must be installed, enabled, exact-versioned, and exact-owner verified.

The installed cache roots under the app-server-reported `<codexHome>/plugins/cache/openai-bundled/<name>/<version>` are hashed with sky-cua's canonical tree algorithm, including POSIX mode bits, file sizes and bytes, directory records, Unicode-code-point path ordering, and hard rejection of symlinks/special files. The reported home is authoritative for supported user/custom home scopes; `<agentDir>/codex-home` is the isolated default. Effective Codex config at the exact target thread CWD and prepared thread config reject standalone `computer-use` collisions. The installed-plugin inventory is also read before every thread and rejects any other enabled plugin that declares `computer-use` or `node_repl`, binding hook preapproval to the verified managed owners rather than a shared server name. Managed inventory identity is the exact `openai-bundled` marketplace plus expected plugin id/name; its reported path may remain the persisted marketplace source after direct install/read has verified the active manifest path and installed cache bytes. Remote-only inventory entries do not expose MCP ownership and remote `plugin/read` can outlive its catalog detail, so their exact `source: remote` identity selects the plugin cache base whose sole materialized version, Codex-recognized manifest identity, and MCP declaration are inspected locally without trusting drift-prone catalog version metadata. Codex atomically replaces that plugin base root on install; missing or ambiguous cache versions and invalid remote identity, manifest, declaration, or contained path fail closed, and a declared managed MCP name remains a collision. Local marketplace entries continue through `plugin/read`. OpenClaw may retain a global standalone `node_repl` MCP for non-Codex consumers, but both Codex CLI and app-server projections filter that entry so `browser-use@openai-bundled` remains the sole Codex `node_repl` owner. Fresh pre-thread MCP status must report both servers with non-null server info. Any resolver, schema, identity, version, enablement, ownership, installed-tree, collision, or MCP startup mismatch fails before thread creation.

Reconciliation is single-flight per app-server client, Codex home, and `(release_id, codex marketplace manifest_sha256)`, while cache mutation across replacement clients is serialized per home. A successful identical generation is reused only within the same app-server process; every replacement client performs native install/read/hash and MCP readiness proof, and every pre-thread attempt rechecks target-CWD and installed-plugin collisions plus MCP status. An active-release rollover serializes behind any in-flight generation and installs/verifies the new bytes again. Native plugin install is the only Computer/Browser MCP ownership path inside the Codex harness; the global non-Codex `node_repl` entry is deliberately retained and filtered at both Codex projection seams. Threads whose caller disables MCP servers retain the installed global plugin state but apply per-thread plugin MCP policy overlays with both managed servers disabled. The exact-hash managed `node_repl` tools are excluded from generic PreToolUse, PostToolUse, and PermissionRequest native hook relay matchers after reconciliation, preventing an interactive hook trust prompt or relay deadlock before calls reach their verified plugin owner. Do not restore startup-time marketplace discovery, curated/stock/local fallback selection, or any other standalone MCP injection into Codex. Keep `codexPlugins` curated account-app policy separate. Do not add migration or status-command machinery for this seam; Bex removes legacy Codex config entries manually.
...
- Command: `pnpm test extensions/codex/src/app-server/managed-native-plugins.test.ts extensions/codex/src/app-server/attempt-startup.test.ts src/agents/cli-runner/bundle-mcp-codex.user-config.test.ts`

Rebase notes:

- Re-check the producer `ActiveRuntimeResolution.as_dict`, `_codex_plugin_contract`, and canonical tree digest before replay. The producer contract and installed release are authoritative; do not infer marketplace locations from source checkouts.
- Re-check pinned sibling `../codex` before replay. Codex `0.144.5` reads `marketplacePath` as the absolute marketplace JSON file, atomically replaces the versioned cache root, enables on install, and includes plugin MCPs in a new thread loaded after reconciliation.
- `config/mcpServer/reload` only refreshes existing threads and is not a pre-thread readiness mechanism. Exact plugin reads, collision-free effective inputs, installed-cache hashes, and global `mcpServerStatus/list` readiness are the pre-thread proof.
- Browser caller provenance, extension socket discovery, IAB exclusion, and legacy IAB-id normalization remain producer-owned behavior inside the managed `browser-use` plugin. Do not reintroduce OpenClaw provenance-selector environment variables or browser discovery policy.
...
### Private root package dependency bundling

Carry behavior: the root npm package declares `@openclaw/fs-safe` in `bundleDependencies`, so private-registry deployment does not depend on that registry separately publishing or caching the patched filesystem runtime. The package tarball checker rejects every declared bundle dependency whose package manifest is absent, and it additionally verifies the bundled `@openclaw/fs-safe` identity/version, every OpenClaw-used export, import closure, and patched `pinned-python`/`pinned-write` runtime files. The Gitea private publish workflow runs that check before publication and Saga deployment.
...
- `test/scripts/check-openclaw-package-tarball.test.ts`
- Actual `pnpm --config.ignore-scripts=true pack` proof followed by `node scripts/check-openclaw-package-tarball.mjs <tarball>` and direct inspection of `package/node_modules/@openclaw/fs-safe/package.json`

Rebase notes:

- Keep `@openclaw/fs-safe` root-owned and bundled while the fork carries its pnpm patch. Do not publish it as a separate private package merely to satisfy the Gitea deployment registry.
- Preserve the generic checker invariant: every name declared through `bundleDependencies` or `bundledDependencies` must have its package manifest in the artifact.
- Keep the `@openclaw/fs-safe` runtime-entry map aligned with root import usage and the carried pnpm patch; a manifest-only or wrong-version bundle must fail before publication.
- The separately staged `@openclaw/ai` strict Docker packaging path remains independent; do not make the private Gitea workflow depend on the public registry state of `@openclaw/fs-safe`.
