...
## Current replay status: 2026-05-01 onto upstream/main `0d2a201b27`

For `bex/replay-upstream-2026-05-01-r6`, this is the upstream-first replay
snapshot. Earlier 2026-05-01 replay worktrees, including the r5 base before
upstream moved, are reference material only; do not land their conflict scars.

- Context gaps in this fresh replay worktree:
  - `CONTINUITY.md` is absent.
  - `NOTES.md` is absent.
- Active product seams on this branch:
  - seam 3: gateway `tts.convert` now resolves agent/channel/account-scoped TTS config before explicit override validation and synthesis.
  - seam 5: Control UI assistant read-aloud now uses gateway `talk.speak` with the resolved agent id instead of browser speech synthesis.
  - seam 6: Telegram transcribed-audio contexts trigger auto-TTS through explicit `MediaTranscribedIndexes` evidence.
  - Discord `audioAsVoice`, shared speech-core voice-note delivery/transcoding, ACP cwd/runtime/persistent binding behavior, Google Gemini TTS prompt fields, and native Codex GPT-5.5/OpenAI-family defaults are already present on upstream `0d2a201b27` and were re-proved instead of reimplemented.
- Support seams carried after re-check:
  - safe-bin trust checks canonical realpaths when the resolver provides them.
  ...
  - ACP runtime capabilities preserve backend-managed runtime option keys for private `acpx-remote` so core does not validate backend-owned `cwd` as a local path.
  - Vitest preserves symlink paths so private linked plugins under `extensions/` run against the parent checkout instead of failing OXC tsconfig discovery through their realpaths.
- Private extension compatibility:
  - `extensions/acpx-remote`, `extensions/codex-transcribe`, and `extensions/memory-maintenance` are restored as local private symlinked plugin repos.
  - `memory-maintenance` private tests were updated for the current Plugin SDK runtime/test-helper surface.
  - `acpx-remote` and `memory-maintenance` private lint issues were cleaned up so the parent all-lane gate can pass with those links present.
- Deferred seams:
  - None in the current r6 replay snapshot. Landing is still intentionally unperformed.
- Validation state so far:
  - `pnpm install` passed in the fresh replay worktree.
  - `git diff --check` passed after the first support seam batch.
  - Safe-bin proof passed:
    - `pnpm test src/infra/exec-safe-bin-trust.test.ts src/infra/exec-approvals-safe-bins.test.ts -t "realpath|safe-bin|trusted"`
  - Telegram auto-TTS proof passed:
    - `pnpm test src/auto-reply/reply/dispatch-from-config.reply-dispatch.test.ts src/auto-reply/reply/dispatch-from-config.test.ts -t "inbound-audio|MediaTranscribed|audio"`
  - Runtime-sidecar private-extension hygiene proof passed:
    - `pnpm test src/plugins/bundled-plugin-metadata.test.ts test/vitest-scoped-config.test.ts -t "runtime sidecar|acpx|baseline|include patterns"`
  - TTS scope proof passed:
    - `pnpm test src/gateway/server-methods/tts.test.ts src/tts/tts-config.test.ts`
  - Speech-core voice-note proof passed:
    - `pnpm test extensions/speech-core/src/tts.test.ts -t "voice-note|transcoded|BlueBubbles|Discord auto TTS"`
  - Control UI read-aloud proof passed:
    - `node --import tsx scripts/control-ui-i18n.ts sync --write`
    - `pnpm test:ui src/ui/chat/grouped-render.test.ts`
    - `pnpm tsgo:test:ui`
  - Cross-channel voice preservation proof passed:
    - `pnpm test src/infra/outbound/deliver.test.ts extensions/telegram/src/bot/delivery.test.ts extensions/whatsapp/src/outbound-base.test.ts extensions/matrix/src/matrix/send.test.ts extensions/feishu/src/media.test.ts -t "audioAsVoice|voice"`
  - Private extension links were restored and runtime-sidecar filtering still passed:
    - `pnpm runtime-sidecars:check`
  - Private symlinked extension validation passed:
    - `pnpm test:extension acpx-remote`
    - `pnpm test extensions/codex-transcribe/index.test.ts extensions/memory-maintenance/src/service.test.ts extensions/memory-maintenance/src/tools.test.ts`
    - `pnpm tsgo:all`
  - Discord voice preservation proof passed:
    - `pnpm test extensions/discord/src/outbound-adapter.test.ts extensions/discord/src/monitor/reply-delivery.test.ts -t "audioAsVoice|voice"`
  - ACP upstream-equivalence proof passed:
    - `pnpm test src/acp/control-plane/manager.test.ts -t "cwd|runtime options|persistent|backend|acpxRecordId|fresh persistent"`
  - Codex/OpenAI-family upstream-equivalence proof passed:
    - `pnpm test src/agents/harness/selection.test.ts extensions/openai/openai-codex-provider.test.ts src/plugins/provider-runtime.test.ts src/commands/models/auth.test.ts -t "gpt-5.5|agentRuntime|codex|openai-codex|native"`
  - Final replay gate passed:
    - `git diff --check`
    - `pnpm check:changed`
  - Landing is intentionally unperformed: do not force-push `origin/main`, realign local `main`, or restart a gateway without explicit landing approval and a fresh upstream preflight.
