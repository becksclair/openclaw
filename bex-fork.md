...
Current replay target: `v2026.7.1-beta.1`.
...
## v2026.7.1-beta.1 seam necessity review

Replayed from fork head `f19cf953bb` (base `v2026.6.11-beta.1`) onto upstream `v2026.7.1-beta.1` (`4eb1d333cf`). 101 commits replayed; none dropped empty. Fifteen commits needed conflict resolution. Net: 33 carries (14 runtime, 15 partial-overlap, 4 support/proof), 3 absorbed upstream, 1 drop candidate.

Three seams newly absorbed by upstream this replay: **Reply session init burst serialization** (upstream shipped `runExclusiveSessionStoreWrite` wrapping `initSessionStateAttemptLocked` per-store; the fork's `replySessionInitializationQueues` per-(store,session) queue was dropped in favor of the upstream serializer), **Gateway main session direct delivery** (`resolveChatSendOriginatingRoute` direct-main guard is now byte-identical upstream), and **Telegram transcribed-audio TTS intent** (upstream `message-action-runner` dispatch preserves the intent; no source carry). The **Native Codex message-tool TTS delivery** and **Telegram** rich-message seams were reconciled onto upstream's `buildThreadParams`/`singleUseReplyTo` rewrite and shared `send-error-predicates` module. **Context-rich realtime Talk tools** / **agent-tools** runtime-policy layering was folded onto upstream's new `capabilityProfile.policy` abstraction. Post-rebase typecheck repairs: `heartbeat-runner.ts` commitment-only returns needed the fork's `hasNotificationEvents`/`hasIgnorableNotificationEvents` fields; a stale `messageThreadId` local and a test `AgentMessage` cast were corrected.

| Seam                                      | Decision              | Importance | v2026.7.1-beta.1 evidence / replay note                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| ----------------------------------------- | --------------------- | ---------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
...
| Gateway main session direct delivery      | Absorbed upstream     | Medium     | resolveChatSendOriginatingRoute direct-main guard (isConfiguredMainSessionScope/canInheritConfiguredMainRoute) HEAD:src/gateway/server-methods/chat.ts:1265-1281 == v2026.7.1-beta.1:1219-1235; seam region diff empty (byte-identical). chat.ts diffs elsewhere are unrelated (final-audio, spawnedBy).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
| Reply session init burst serialization    | Absorbed upstream     | High       | Fork symbol replySessionInitializationQueues absent in both refs. HEAD session.ts:322 uses runExclusiveSessionStoreWrite wrapping initSessionStateAttemptLocked(:328) — init region 305-335 byte-identical to v2026.7.1-beta.1 session.ts; config/sessions/store-writer.ts identical between refs.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
...
| Control UI read aloud through Talk        | Runtime carry         | Medium     | HEAD ui/src/ui/chat/talk-tts.ts exports speakText/stopTts/isTtsSupported; app.ts:99 imports speakText, handleReadAloud() at ~1370 calls it; grouped-render.ts:773 renders aria-label="Read aloud". Upstream v2026.7.1-beta.1: talk-tts.ts absent (git cat-file fatal), no speakText; grouped-render.test.ts:549 asserts Read-aloud button toBeNull (HEAD test:555 asserts present).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| Discord 30032 command deploy recovery     | Runtime carry         | Medium     | HEAD provider.deploy-errors.ts:376 isDiscordDeployCommandLimit (code 30032, :383); provider.deploy.ts:159 falls back to deployCommands({mode:"overwrite",force:true}). Upstream v2026.7.1-beta.1 deploy-errors.ts has only isDiscordDeployDailyCreateLimit(30034):360, no 30032; deploy.ts:142 only mode:"reconcile".                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
| Docker replay validation directives       | Support/proof carry   | Medium     | HEAD:AGENTS.md L127-128 (clean Docker validation container / test:docker:local:all lane) + L139-140 (no mounting Bex's real ~/.openclaw, credentials, Gateway state, private plugin/session data). Upstream v2026.7.1-beta.1:AGENTS.md grep count 0 for 'Bex fork replay\|test:docker:local:all\|mounting Bex\|openclaw-fork-replay'; no openclaw-fork-replay ref anywhere in tag tree. Docs/policy only, no runtime code.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| Codex app-server force full access        | Runtime carry         | Medium     | HEAD adds env toggle + clamp: config.ts:253 isCodexAppServerForcedFullAccess / :263 clampCodexAppServerRuntimeToFullAccess (networkProxy=undefined, never, danger-full-access, user) applied at resolver :739-740, promotion bail app-server-policy.ts:26, conversation-binding.ts:444,652, side-question.ts:266. v2026.7.1-beta.1: git grep OPENCLAW_CODEX_FORCE_FULL_ACCESS + those symbols = empty; upstream has only danger-full-access enum + guardian-driven selectForcedDangerFullAccessSandbox, no env force-clamp.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
| Telegram transcribed-audio TTS intent     | Absorbed upstream     | Medium     | Telegram transcribe body extensions/telegram/src/bot-message-context.body.ts IDENTICAL HEAD vs tag. inboundAudio TTS-intent threading already in v2026.7.1-beta.1: message-tool.ts:1483, message-action-runner.ts:139/1143, message-action-tts.ts:49/77. No fork source for this seam.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| Agent-scoped TTS conversion config        | Drop candidate        | Low        | src/tts/tts-config.ts: resolveEffectiveTtsConfig (L126) deep-merges resolveAgentTtsOverride(cfg,agentId) (L43) over messages.tts. Byte-identical in HEAD and v2026.7.1-beta.1 (same L43/L126); git diff tag..HEAD leaves these fns untouched. Upstream full, no fork carry.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
### runtime.llm.complete reasoning effort

Carry behavior: `runtime.llm.complete` (the plugin-facing LLM completion surface, `PluginRuntimeCore.llm.complete`) gains an optional `reasoning?: LlmCompleteReasoning` param (`"off" | "minimal" | "low" | "medium" | "high" | "xhigh" | "adaptive" | "max"`), forwarded into the simple-completion `options.reasoning` inside `createRuntimeLlm().complete` (`src/plugins/runtime/runtime-llm.runtime.ts`). Without it, plugin completions never set a reasoning option, so reasoning-capable models (e.g. `openai/gpt-5.3-codex-spark` over the ChatGPT-OAuth `/responses` backend) fall through the transport's `effort:"none"` default branch and send no `reasoning` field. The out-of-tree `voice-emotion` plugin uses this to pin its annotation model to `medium` via its own `reasoningEffort` config. There is NO config-only seam for this on the completion path (traced: `model.params`/`thinkingLevelMap`/`plugins.entries.*.llm` are all inert here), so it must be passed as the option. Additive/optional and upstream-PR candidate. Type: `LlmCompleteParams.reasoning` in `src/plugins/runtime/types-core.ts`; honored by `completeWithPreparedSimpleCompletionModel` → `normalizeSimpleCompletionReasoning` → `buildOpenAIResponsesParams` (sets `reasoning.effort`).

### Voice Emotion plugin (out-of-tree seam consumer)

The two carries above (`tts_prepare` + the Google `personaPrompt` override, and `runtime.llm.complete` reasoning) exist to serve one feature: the `voice-emotion` plugin — a separate git-installed repo at `~/projects/voice-emotion`, wired through `plugins.entries.voice-emotion` — which preprocesses every agent voice note through an LLM to add provider-appropriate emotional delivery before synthesis. It is documented here because the seams are inert without their consumer, and the plugin's integrity contract (fail-open; never corrupt spoken text) is what justifies the seam shapes (transform-only hook, director's-notes-only Google merge, additive reasoning option).
...
Live wiring (`openclaw.json`). Annotation model `openai/gpt-5.3-codex-spark` (the ChatGPT-OAuth `/responses` backend — faster and cheaper than API models and included in the subscription; unblocked purely via the plugin's `allowedModels`, no seam patch) at `reasoningEffort: medium`, with `openai/gpt-5.4-mini` as fallback. Sky's persona routes to ElevenLabs `eleven_v3` (inline-tags), as does Luke. The plugin owns its own tests (`npm test` in `~/projects/voice-emotion` — 72 cases over the guards, orchestration, and fail-open paths); the fork-side seam tests are listed under the `tts_prepare` section above.
