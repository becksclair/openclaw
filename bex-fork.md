...
Current replay target: `v2026.7.1-beta.3` (`2b47b1c460`) plus security fixes #102403 and #102398.
...
## v2026.7.1-beta.3 seam necessity review

Replayed from fork head `22faa88c4f` onto upstream `v2026.7.1-beta.3` (`2b47b1c460`), with post-target security fixes #102403 and #102398 applied before fork seam resolution.
...
| Auto-TTS excludes tool delivery kind                                 | Runtime carry         | Medium     | `packages/speech-core/src/tts.ts` `maybeApplyTtsToPayload` skips `kind: "tool"` independent of resolved mode, so `mode: "all"` never speaks tool chrome. Tool payloads carrying real audio are promoted to `kind: "final"` in `chat.ts` before delivery. Prove: `pnpm test packages/speech-core/src/tts.test.ts`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
...
| runtime.llm.complete reasoning effort                                | Runtime carry         | Medium     | `LlmCompleteParams.reasoning` (`src/plugins/runtime/types-core.ts`) forwarded into `options.reasoning` in `createRuntimeLlm().complete` (`runtime-llm.runtime.ts`); without it reasoning-capable models fall through the transport's `effort:"none"` default and send no `reasoning` field. Consumer: `voice-emotion` pins its annotation model to `medium`. Additive/optional; upstream-PR candidate.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| Voice Emotion plugin (out-of-tree seam consumer)                     | Support/proof carry   | Medium     | Out-of-tree `~/projects/voice-emotion` binds `api.on("tts_prepare", handler)` (the TYPED hook table, NOT legacy `api.registerHook`, which the typed dispatcher silently ignores) and resolves `runtime.llm.complete` lazily per invocation. No repo change; the two producer seams above are inert without it. Rebuild and run its 72-case suite after the producers re-land.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
...
| Tokenjuice fork runtime package                                      | Runtime carry         | High       | OpenClaw's bundled `@openclaw/tokenjuice` plugin otherwise restores upstream `tokenjuice@0.8.1` during ordinary pnpm or Gateway UI dependency reconciliation. Carry a reproducible pnpm patch generated from committed sibling fork `c8e3fbb`, so clean clones and frozen installs receive the same bounded OpenClaw/Codex runtime bytes without requiring the sibling checkout.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
...
Carry behavior: the bundled `@openclaw/tokenjuice` plugin must execute the runtime produced by Bex's committed Tokenjuice fork, including bounded OpenClaw fallback and Codex-native PreToolUse wrapping. OpenClaw installs the fork runtime as a pnpm patch over public `tokenjuice@0.8.1`; this keeps the dependency graph reproducible for clean clones while the sibling fork remains the source and test owner.

Primary seam files:

- `patches/tokenjuice@0.8.1.patch`
- `pnpm-workspace.yaml`
- `pnpm-lock.yaml`
...
- SHA-256 checks for installed `dist/index.js` and `dist/hosts/codex/index.js` must match the build from sibling commit `c8e3fbb`.
...
- Regenerate the pnpm patch from the packed sibling fork after every Tokenjuice fork commit or upstream version bump; do not hand-edit compiled runtime hunks.
- Keep `extensions/tokenjuice/package.json` on the public `tokenjuice` version. The pnpm patch is Bex-fork install policy, not a published plugin dependency contract.
