...
Carry behavior: Saga's npm configuration resolves `@openclaw/*` through the private Gitea registry while ordinary dependencies resolve through npmjs. Before publishing the root package, the workflow derives the complete OpenClaw-owned direct dependency set from `package.json`, packs `@openclaw/ai` from its workspace and the exact installed `@openclaw/fs-safe` and `@openclaw/proxyline` runtimes, verifies every packed name/version, and publishes them first. Workspace packages receive an immutable per-run private version and the root manifest (plus any matching shrinkwrap entry) is rewritten to that exact version after publication; fixed external versions must match the freshly packed SHA-512 integrity. The root still bundles patched fs-safe so public npm and strict Docker artifacts retain the fork patch independently of Gitea.

Deployment directive: build, pack, verify, and publish the private dependency closure and root `openclaw` package from this development machine before invoking Saga deployment. Do not wait for or ask Saga to build or publish packages. Saga's role starts at installing the already-published root version, then restarting OpenClaw and proving the installed version, commit, and health.
...
- `test/scripts/pack-private-npm-dependencies.test.ts`
- Actual `node scripts/pack-private-npm-dependencies.mjs pack <output-dir> <run-id>` proof followed by direct inspection of all three dependency tarball manifests and `rewrite-root` proof for the run-versioned AI dependency
...
- Preserve the machine boundary: package construction and registry publication happen here; Saga only installs and validates the immutable published artifact. A CI workflow may automate the same steps, but it must not move build ownership onto Saga or make Saga deployment wait on a Saga-side build.
